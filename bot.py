...
def find_matches(chat, query):
    ...
    for x in games:
        h = x['teams']['home']['name']
        a = x['teams']['away']['name']
        if q in h.lower() or q in a.lower() or q in f'{h} {a}'.lower():
            matches.append(x)
    if not matches:
...
def match_card(chat, fid):
    ...
    if not fs:
        return tg('sendMessage', chat_id=chat, text='❌ Матч не найден.')
    f = fs[0]
    h = f['teams']['home']
    a = f['teams']['away']
    ...
    tg('sendMessage', chat_id=chat, text=text, reply_markup={'inline_keyboard': [[
        {'text': '🚀 ОТКРЫТЬ RUSTAM OS', 'web_app': {'url': url}}
    ]]})
...
def live(chat):
    ...
    if not games:
        return tg('sendMessage', chat_id=chat, text='🔴 Сейчас live-матчей нет.')
    buttons = []
    for x in games[:30]:
        h = x['teams']['home']['name']
        a = x['teams']['away']['name']
        hs = x['goals']['home'] if x['goals']['home'] is not None else 0
        aws = x['goals']['away'] if x['goals']['away'] is not None else 0
        minute = x['fixture']['status'].get('elapsed') or 0
...
def handle_message(m):
    ...
    if text == '🔴 Live':
        return live(chat)
    if text == '🔎 Анализ':
        return tg('sendMessage', chat_id=chat, text='🔎 Напиши название команды для поиска сегодняшнего матча.\n\nНапример: Real Madrid')
    if text == '🎯 Сигналы':
        return tg('sendMessage', chat_id=chat, text='🎯 SIGNAL ENGINE\n\nСначала выбираем реальный матч. Затем RUSTAM OS показывает прогноз и уровень риска в Mini App.')
    if text == '📊 Таблицы':
        return standings(chat)
    if text == 'ℹ️ Статус':
        return tg('sendMessage', chat_id=chat, text='🧠 RUSTAM OS ONLINE\n\n⚽ API-Football: ' + ('ONLINE' if API_KEY else 'NO KEY') + '\n🚀 Mini App: ONLINE\n🔐 API key: server-side')
    if text in ('🧠 AI', '📊 Аналитика', '💼 Работа', '⚙️ Настройки'):
        return tg('sendMessage', chat_id=chat, text='🚧 Этот модуль подключим следующим релизом. Football Intelligence уже работает.')
    if text:
        return find_matches(chat, text)
    ...
    try:
        ...
        for u in data.get('result', []):
            ...
            if 'callback_query' in u:
                ...
                cb = q.get('data', '')
                if cb.startswith('match:'):
                    match_card(chat, cb.split(':', 1)[1])
            elif 'message' in u:
